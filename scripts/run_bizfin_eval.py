"""
Batch evaluation runner for BizFinBench.v2 datasets.
...
Usage:
    python -m scripts.run_bizfin_eval \
        ...
        --output /tmp/bizfin_summary.json \
        --limit 10

Reads a JSONL via BizFinBenchProvider, generates tasks with DynamicTaskGenerator,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch-evaluate BizFinBench.v2 tasks.")
    parser.add_argument(
        "--dataset-path",
        required=True,
        help="Path to BizFinBench.v2 directory (e.g., data/BizFinBench.v2)",
    )
    parser.add_argument(
        "--task-type",
        required=True,
    ...
    parser.add_argument(
        ...
        default="en",
        choices=["en", "cn"],
        help="Language: 'en' or 'cn' (default: en)",
    )
    parser.add_argument(
        "--purple-endpoint",
        required=True,
        help="Purple Agent endpoint URL (e.g., http://localhost:9110)",
    )
    parser.add_argument(
        "--purple-agent-id",
...
def _load_templates(args: argparse.Namespace):
    """Load templates from BizFinBench provider."""
...
def main() -> None:
    ...
    print(f"Loaded {len(templates)} templates from BizFinBench.v2")
    ...
    async def run_all():
        ...
        for idx, tpl in enumerate(templates):
            print(f"Processing {idx + 1}/{len(templates)}: {tpl.template_id}")
            try:
    ...
    try:
        ...
        fallback = "/tmp/bizfin_summary.json"
